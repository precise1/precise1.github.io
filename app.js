...
function showAll (){
  ...
  for (i=0; i < videosList.length ; i++) {
    ...
    let x = generateRowHTML(videosList[i])
    y.push(x)
...
function searchVideo() {
  ...
  let input = document.querySelector("#txt-search").value

  //gave up on lower case functionality. Only functions on specific entries.
  ...
  let ctr=0
    ...
    //loop through output array searching for text inputed.
    for (i=0; i < videosList.length; i++){
      x = videosList[i].description.includes(input)
        //testing output
        //console.log(`${x}`)
      
      if (x==true){

            //add found results to result array to later output
      ...
      if (ctr>0){
        for (i=0; i < result.length; i++) {
          ...
          let x = generateRowHTML(result[i])
          output.push(x)
      ...
      document.querySelector("#txt-search").value=""
        ...
        document.querySelector("#results").innerHTML=`${output}`
        document.querySelector("#txt-search").value=""
          alert(`No videos found with the keyword ${input}`)
...
//function uploadVid(){
 ...
 /* document.querySelector(".container").innerHTML=`<form action="https://formspree.io/f/meqvodqr" method="POST">
...
//document.querySelector("#btn-upload").addEventListener("click", uploadVid )

//document.querySelector("btn.btn-secondary").addEventListener("click", uploadVid)
